...
import os
from semchunk import chunk # type: ignore
import ast
from typing import Any
from tqdm import tqdm

def chunking(size: int, overlap: int) -> tuple[list[dict[str, Any]], list[MinimalSource]]:
    corpus = list()
    chunks = list()
    for thing in tqdm(glob('data/raw/vllm-0.10.1/**', recursive=True), "chunking..."):
        # print(thing)
        if os.path.isfile(thing):
            if thing.endswith(".md") or thing.endswith(".txt"):
                with open(thing, 'r') as f:
                    ...
                    balise = 0
                    for t in tmp:
                        new_chunk, new_corp, balise = create_obj(thing, text, t, balise)
                ...
                with open(thing, 'r') as f:
                    ...
                    lines_left = [line for i, line in enumerate(lines) if i not in covered]
                    if lines_left:
                        part = str()
...
def create_obj(filepath: str, src: str, text: str, balise: int) -> tuple[
    MinimalSource, dict[str, Any], int]:
